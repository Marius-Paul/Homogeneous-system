...
def create_functions(V1, c, N, t, t_diag, matrix_list, kBT, U):
    ...
    @vectorize
    def E_k(ek, gkd, Delta_d, mu):
        return sqrt((ek - mu) ** 2 + (Delta_d * gkd) ** 2)
    ...
    def sum_over_all_k_for_E(x):
        ...
        return 4.0 * res_vec[0] + 2.0 * res_vec[1] + 2.0 * res_vec[2] + res_vec[3]


    def calc_E(n_el, x):
        sum_res = sum_over_all_k_for_E(x)
        return sum_res / N - 4.0 / V1 * (x[1] ** 2 - x[0] ** 2) + x[2] * (n_el - 1.0)
