...
# x = [W, Delta_d, mu, L]
x_test = array([-0.1435, -0.1675235, -2.65, 0.4564])
...
@vectorize
def E_k(ek, gkd, Delta_d, mu, L):
    return sqrt((ek - mu) ** 2 + (Delta_d * gkd + L) ** 2)
...
def sum_over_all_k_for_f(x, sym):
    ...
    if sym == 8:
        ...
        Z2 = y3
        Z3 = y6
        return array([sum(Z0), sum(Z1), sum(Z2), sum(Z3)])
    else:
        print('symmetry not implemented')
        return
