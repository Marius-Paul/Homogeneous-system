...
def minimizing_routine(W_Delta_0_Delta_mu, n_el, t_diag, K, U, eta_k_alpha, damping_fac):
    ...
    while delta_x>min_acc:
        ...
        mu_start = mu_start + 0.5 * (n_el - calc_n(Ak, Ek, Tk))
    ...
    return W_start, Delta_0_start, Delta_start, mu_start



def GA_minimizing_routine(W_Delta_0_Delta_mu, n_el, t_diag, K, U, eta_k_alpha, damping_fac, EPD_start):
    W_start, Delta_0_start, Delta_start, mu_start = W_Delta_0_Delta_mu
    delta_x = 1.0
    cnt_GA = 0
    lamda = 1.0e4
...
alpha_vec = array([0.0, 1.0e-2, 0.5-1.0e-2, 0.5])

def solve_HF(U, t_diag, x_start):
    ...
    for indc in range(len(alpha_vec)):
        ...
        x_result = minimizing_routine(x_start[indc], n_el, t_diag, 1.0, U, eta_k_alpha, damping_fac)
...
def solve_GA(U, t_diag, x_start, Jz):
    damping_fac = 0.5 / (U + 1.0)
    ...
    for indc in range(len(alpha_vec)):
        ...
        DO = calc_DO(EPD_start[2], Jz, J)
        eta_k_alpha = calc_eta_k_alpha(alpha_vec[indc])
        x_result = GA_minimizing_routine(x_start[indc], n_el, t_diag, K, U, eta_k_alpha, damping_fac, EPD_start)
        x_result_vec.append(x_result)
        energy_vec[indc] = GA_calc_energy(alpha_vec[indc], n_el, t_diag, U, x_result[0], x_result[1],
                                          x_result[3], x_result[2], EPD_start, DO, K, lamda3)
...
def calc_EPD_start_values(n_el, Delta_start):
    ...
    return array([E_start, P_start, D_start])
...
if calc_stuff:
    ...
    for n_el_array_index, n_el in enumerate(n_el_array):
        ...
        if n_el_array_index == 0:
            ...
            mu_start_d = new_x_start_n_el[1][3]
            W_start_d = new_x_start_n_el[1][0]
            Delta_0_start_d = new_x_start_n_el[1][1]
            Delta_start_d = new_x_start_n_el[1][2]
        ...
        for t_diag_index, t_diag in enumerate(t_prime_array):
            ...
            for U_index, U in enumerate(U_array):
                ...
                symmetry, x_start = solve_HF(U, t_diag, x_start)
                if n_el_array_index == 0:
