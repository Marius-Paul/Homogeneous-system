...
# x = [W, Delta_d, mu, cidciu, lamda]
x_test = array([-0.24, -0.09, -1.32, 0.0, 0.0])
...
@vectorize
def E_k(ek, gkd, Delta_d, mu, lamda):
    return sqrt((ek - mu) ** 2 + (Delta_d * gkd + lamda) ** 2)
...
def calc_E(D, n_el, x):
    ...
    return sum_res / N - 4.0 / V1 * (x[1] ** 2 - x[0] ** 2) + x[1] * (n_el - 1.0) + U * D - 2.0 * x[3] * x[4]
...
def calc_y_for_symmetry_test(x, n_el, D):
    ...
    y2 = E_k(y1, y0d, x[1], x[2], x[4])
    y3 = (y1 - x[2]) / y2
    y4 = y0d * (x[1] * y0d + x[4]) / y2
    y5 = tanh(y2 / (2.0 * kBT))
    y6 = (x[1] * y0d + x[4]) / y2 * y5
